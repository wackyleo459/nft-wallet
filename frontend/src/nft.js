...
export function fetchNftRouter(canister, index) {
    return (context, trigger) => {
        fetchNft(context.params[canister], Number.parseInt(context.params[index])).then(nft => {
...
export async function fetchNft(principal, index, checkOwner) {
    if (checkOwner !== false && !await isOwner({canister: principal, index})) {
        return null;
    }
    ...
    if (!nft.preview) {
        nft.preview = nft.content?.find(part => part.contentType.startsWith('image'));
    }
    return nft;
}
...
export async function fetchCollectionInfo(principal) {
    ...
    const icon = URL.createObjectURL(new Blob([Base64.toUint8Array(logo.data)], {type: logo.logo_type}));
...
async function getAuthClient() {
    if (!authClient) {
        ...
        dipCanister = dipCanisterFactory({identity: authClient.getIdentity(), ...hostOptions});
    }
    return authClient;
}
...
export async function transfer(nft, to, notify) {
    const walletCanister = await getWalletCanister();
    const walletNft = { canister: Principal.fromText(nft.canister), index: nft.index };
    await walletCanister.transfer(walletNft, Principal.fromText(to), notify == null ? [] : [!!notify]);
}

export async function register(canister, index) {
    const walletCanister = await getWalletCanister();
    const res = await walletCanister.register({canister: Principal.fromText(canister), index});
    if ('Err' in res) {
        throw res.Err;
    }
}
